...
object FunSets {
   ...
   */
    def union(s: Set, t: Set): Set =  x =>  if (contains(s,x) || contains(t,x)) true else false
   ...
   */
    def filter(s: Set, p: Int => Boolean): Set = x => if (contains(s,x) && p(x)) true else false
   ...
   */
    def exists(s: Set, p: Int => Boolean): Boolean = {
      for (i <- -bound to bound)  if (contains(s, i) && p(i) ) return true
      false
    }
